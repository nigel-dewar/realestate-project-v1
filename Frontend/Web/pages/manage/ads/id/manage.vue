    ...
    <div class="card m-t-md">
      <div class="card-content">
        <div class="columns">
          <div class="column is-6">
            ...
            <p class="is-size-6 is-size-7-mobile">
              <b>Ad Ref: </b> <span>{{ listing.listingRef }}</span>
            </p>
            <p class="is-size-6 is-size-7-mobile">
              <b>Dates:</b>
              <span>{{ dateListingStarts }}</span> - <span>{{ dateListingEnds }}</span>
          ...
          <div class="column is-6">
            <div class="level is-mobile">
              ...
              <div class="level-left">
                <div class="level-item has-text-centered">
                  <div>
                    <p class="is-capitalized has-text-weight-medium">Ad Info</p>
                    <p class="tag" :class="listingComplete.class">
                      {{ listingComplete.text }}  
                      <!-- 2 / 3 Complete -->
                    </p>
                    <!-- <b-progress type="is-success" :value="80" show-value>
                      80%
                    </b-progress> -->
                  </div>
                ...
                <div class="level-item has-text-centered">
                  <div>
                    <p class="is-capitalized has-text-weight-medium">Status</p>
                    <div class="tag" :class="listingStatus.class">
                    {{ listingStatus.text }}
                    </div>
                  </div>
                </div>
              ...
              <div class="level-right">
                <div class="level-item">
                  ...
                  <!-- <button class="button is-primary m-r-sm is-medium" @click="viewLiveAd()" v-if="listing.isPublishedLive">View Live Ad</button> -->

                  <Modal
                    v-if="userRequestOptions.canConfirm"
                    @submitted="confirm($event, 'confirm')"
                    openTitle="Start"
                    openBtnClass="button is-success is-medium"
                    title="Confirm Ad?"
                    :showButton="true"
                    actionTitle="Confirm"
                  >
                    <div>
                      ...
                      public. You will not be able to edit the Start and End
                      dates of the Ad after confirming.
                    </div>
                  </Modal>
                  ...
                  <Modal
                    ...
                    openTitle="Cancel"
                    openBtnClass="button is-danger is-medium"
                    title="Cancel Ad?"
                    :showButton="true"
                    actionTitle="Confirm"
                  >
                    <div>
                      When you cancel your Ad, it will ensure it will not appear
                      to the public. To Re-list your Ad, you can use the Re-list
                      button to re-activate it, and set new dates for the Ad to
                      run.
                    </div>
                  </Modal>
            ...
            <!-- <div v-if="listing.active">
              <span class="tag is-info"
                >Note: Changes made to an Active Ad can take up to 2 hours to
                display</span
              >
            </div> -->
          </div>
        ...
        <div class="has-text-centered">
          <b-message :type="listingStatus.class"><span class="is-size-7">{{listingStatus.desc}} </span></b-message> 
          <button class="button is-primary is-medium" @click="viewLiveAd()" v-if="listing.isPublishedLive">View Live Ad</button>
        </div>
      </div>
    </div>

    <!-- <div class="m-t-md m-b-md has-text-centered" v-if="changes">
      <div>
        <button
              v-if="listing"
    ...
    <div class="m-t-sm">
      ...
      >
        <b-tab-item
          ...
          :value="1"
        />
        <b-tab-item label="Property" icon="home" :value="2" />
        <b-tab-item label="Pics" icon="account" :value="3" />
      </b-tabs>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      activeTab: 0,
      changes: false,
      steps: ['AdDetails', 'Property', 'Images'],
      activeStep: 1,
      loaded: false,
      canProceed: false,
      form: {
  ...
  computed: {
    ...
    ...mapGetters({
      ...
      listingComplete: 'manage/listings/listingComplete'
    }),
    formInValid() {
  ...
  methods: {
    ...
    viewLiveAd() {
      let routeUrl = this.$router.resolve({
          path: `/details/${this.listing.slug}${this.listing.listingRef}`
     });
     window.open(routeUrl .href, '_blank');
      // this.$router.push(`/details/${this.listing.slug}${this.listing.listingRef}`)
    },
    switchChange() {
      alert('changed')
    },
    ...
    //   }
    // },
    mergeFormData: _.debounce(function({ data, isValid }) {
