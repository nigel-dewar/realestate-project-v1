  ...
  <section class="section">
    <div class="card">
      <div class="card-content">
        <p class="title">Add Pictures</p>
        <p class="subtitle">You can add up to 20 pictures</p>
      <p>You will need to add at least 1 picture to proceed</p>
      <p>Main pic exists: {{mainImageExists}}</p>
      {{isValid}}
      </div>
      <div class="card-content">

        <div class="inputImage" v-if="!displaySlim">
          <button class="button is-success" @click="displaySlimControl">Upload or Take Photo</button>
        </div>

        <div class="slim-container" v-if="displaySlim">
          <slim-cropper :options="slimOptions">
            <input type="file" accept="image/*" id="image-picker" ref="slimFilePicker" />
          </slim-cropper>
        ...
        <!-- <div class="image-editor" v-if="editMode">
          <div class="image-editor__image">
            <img :src="editingImage.url" alt="">
          </div>
          <div class="image-editor__menu">
            <button class="button is-small is-primary" @click="editImage(image)">
              <span class="m-r-xs">Edit</span>
      ...
      <div class="images" v-if="images">
        <div class="images-container" v-for="(image, index) in images" :key="image.id" >
          ...
          <div class="images-container__menu">
            ...
            <button class="button is-small is-primary" @click="setMain(image, index)" v-if="!image.isMain">
              <span class="m-r-xs">Main</span>
              <icon name="heart" size="small" /> 
            </button>
            <button class="button is-small is-danger" @click="deleteImage(image, index)" v-if="!image.isMain">
              <!-- <span class="m-r-xs"></span> -->
              <icon name="trash-alt" size="small" /> 
            </button>
            <i v-if="editLoading && target == index" class="fas fa-spinner fa-spin"></i>
...
export default {
  ...
  data() {
    return {
      ...
      slimOptions: {
        ...
        maxFileSize: 100, // value is 2MB limit
      },
      uploading: false,
      editLoading: false,
      error: '',
  ...
  computed: {
    ...
    mainImageExists() {
      return this.images.filter(x=>x.isMain == true).length > 0 ? true : false
    },
    mainImage() {
